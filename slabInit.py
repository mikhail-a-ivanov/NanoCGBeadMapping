...
def SlabGeometry(atoms, shift):
    # Get the extreme slab coordinates and save them to the corresponding X, Y, Z lists:
    # Add a shift value to the min X,Y,Z and remove it from the max X,Y,Z

    X = []
    X.append(np.amin(atoms.T[0]) + shift)
    X.append(np.amax(atoms.T[0]) - shift)
    
    Y = []
    Y.append(np.amin(atoms.T[1]) + shift)
    Y.append(np.amax(atoms.T[1]) - shift)
    
    Z = []
    Z.append(np.amin(atoms.T[2]) + shift)
    Z.append(np.amax(atoms.T[2]) - shift)
    
    # Note that the coordinates are in nm
    
    return(X, Y, Z)

def GenerateCGbeadCenters(atoms, x_points, y_points, z_points, shift):
    
    # Get slab extreme coordinates [[x_min, x_max],[y_min, y_max],[z_min, z_max]]
    slab_geometry = SlabGeometry(atoms, shift)
...
def Initialize(filename, resname, x_points, y_points, z_points, shift):
    ...
    CGbeads, spacings = GenerateCGbeadCenters(atoms, x_points, y_points, z_points, shift)
   
    print("Using", x_points,"x", y_points,"x",z_points, "grid for CG centers...")
    print("Using", shift, "nm shift for the CG grid edges...")
